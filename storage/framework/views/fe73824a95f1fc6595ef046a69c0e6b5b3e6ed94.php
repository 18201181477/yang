...
<div class="col-md-9">
	...
	<div class="alert alert-danger alert-dismissible" role="alert">
		<button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
		<strong>提示：</strong> <a href=""><?php echo e(Session::get('pay')); ?></a>
	</div>
	<?php endif; ?>
	<!-- 自定义内容 -->
				...
				<?php foreach($param as $val): ?>
					<tr>
						...
						<?php if(\Auth::user()->status==0): ?>
							<?php 
								$is_or = Queque::where(['user_id'=>\Auth::user()->id])->get()->toArray();
							?>
							<?php foreach($is_or as $v): ?>
								<?php if($v['ke_id']!=$val->id): ?>
								<a href="<?php echo e(url('/queque',['id',$val['id']])); ?>">挂号</a>
								<?php else: ?>
								<a href="<?php echo e(url('/queque',['id',$val['id']])); ?>">取消挂号</a>
								<?php endif; ?>
							<?php endforeach; ?>
						<?php else: ?>
